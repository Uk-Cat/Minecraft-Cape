...
const CAPE_GUIDES = [
  ...
  { x: 11, y: 0, w: 10, h: 1, label: 'Bot' },
];

const ELYTRA_GUIDES = [
  { x: 24, y: 2, w: 10, h: 20, label: 'E.Front' },
  { x: 36, y: 2, w: 10, h: 20, label: 'E.Back' },
  { x: 22, y: 2, w: 2, h: 20, label: 'EL' },
  { x: 34, y: 2, w: 2, h: 20, label: 'ER' },
  { x: 24, y: 0, w: 10, h: 2, label: 'ETop' },
  { x: 34, y: 0, w: 10, h: 2, label: 'EBot' },
];

// Always show both cape and elytra guides together
const ALL_GUIDES = [...CAPE_GUIDES, ...ELYTRA_GUIDES];
...
const PixelCanvas = forwardRef<PixelCanvasHandle>(
  function PixelCanvas(_props, ref) {
    ...
    // --- Generate brush pixel positions for a given center and size ---
    const getBrushPixels = useCallback(
      (cx: number, cy: number, size: number): { x: number; y: number }[] => {
        const half = Math.floor(size / 2);
        const offsetX = size % 2 === 0 ? half - 1 : half; // center for odd, shift for even
        const offsetY = size % 2 === 0 ? half - 1 : half;
        const result: { x: number; y: number }[] = [];
        for (let dy = 0; dy < size; dy++) {
          for (let dx = 0; dx < size; dx++) {
            const px = cx - offsetX + dx;
            const py = cy - offsetY + dy;
            if (px >= 0 && px < CAPE_WIDTH && py >= 0 && py < CAPE_HEIGHT) {
    ...
    const renderOverlayCanvas = useCallback(() => {
      ...
      if (
        ...
        // Determine the brush dimensions for cursor display
        const isBrushTool = currentTool === 'pencil' || currentTool === 'eraser';
        const effectiveBrushSize = isBrushTool ? brushSize : 1;
        const half = Math.floor(effectiveBrushSize / 2);
        const offsetX = effectiveBrushSize % 2 === 0 ? half - 1 : half;
        const offsetY = effectiveBrushSize % 2 === 0 ? half - 1 : half;
        const brushPixelW = effectiveBrushSize * pixelSize;
        const brushPixelH = effectiveBrushSize * pixelSize;
        const brushX = (pos.x - offsetX) * pixelSize;
        const brushY = (pos.y - offsetY) * pixelSize;
    ...
    }, [currentTool, currentColor, brushSize, pixelSize, canvasWidth, canvasHeight]);
    ...
    const handleMouseDown = useCallback(
      (e: React.MouseEvent) => {
        ...
        if (currentTool === 'pencil') {
          const brushPixels = getBrushPixels(pos.x, pos.y, brushSize);
          setPixelBatch(brushPixels.map((p) => ({ x: p.x, y: p.y, color: currentColor })));
          lastDrawnPixel.current = pos;
        } else if (currentTool === 'eraser') {
          const brushPixels = getBrushPixels(pos.x, pos.y, brushSize);
          setPixelBatch(brushPixels.map((p) => ({ x: p.x, y: p.y, color: null })));
          lastDrawnPixel.current = pos;
    ...
    const handleMouseMove = useCallback(
      ...
      [currentTool, currentColor, brushSize, getGridPos, getBrushPixels, setPixelBatch, renderOverlayCanvas]
    );
